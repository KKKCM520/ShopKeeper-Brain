...
class MilvusImportNode(BaseNode):
    ...
    def _validate_state(self,state: ImportGraphState):
        ...
        if not chunks or not isinstance(chunks, list):
            self.logger.error("chunks空或不是list")
            raise StateFieldError(node_name=self.name,field_name="chunks",expected_type=list)
        #3.遍历所有chunk,校验chunk是否胃dict,chunk是否有向量,如果没有就将该chunk丢弃
        final_chunks = []
        for chunk in chunks:
...
if __name__ == '__main__':
    json_path = r"E:\atguigu\demo\workspace_pycharm\ShopKeeper-Brain\knowledge\processor\import_processor\output_dir\万用表RS-12的使用\chunks_vector.json"
